 ...
 * graph instance.
 * <p>
 * It will first visit the object graph then produces a linked list of
 * {@link Node}.
 * <p>
 * The Map Reduce pattern will then be applied from this linked list. Users can
 * create their Map Reduce jobs by using the API provided.
 * <ul>
 * <li> {@link Mapper} : a mapper from the Map Reduce design pattern
 * <li> {@link Reducer} : a reducer from the Map Reduce design pattern
 * <li> {@link Job} : a coherent set of one {@link Mapper} and one or more
 * {@link Reducer}s
 * <li> {@link MapReduce} : a coherent set of one {@link Mapper} and one or more
 * {@link Reducer}s
...
public class UniversalVisitor {
	...
	public void visit(AnnotatedElement ae, Filter filter,
		...
		for (LocalNode node : nodes) {			
			for (MapReduce mapReduce : job.mapReduces()) {
				if (mapReduce.getMapper().handle(node.annotatedElement())) {
					...
					for (Reducer<Object, Object> reducer : mapReduce
							.getReducers()) {
						reducer.collect(t);
					}
				}
	...
	private static class LocalNode extends NodeDefault {
		...
		protected LocalNode(Object instance,
				AnnotatedElement annotatedElement, int level) {
		...
		public String toString() {
			...
			String rep = String
					.format("%sLocalNode [ %s@%s , level=%s , annotatedElement=%s] \n",
							indentation, instance().getClass().getSimpleName(),
							Integer.toHexString(instance().hashCode()),
							level(), annotatedElement());
			return rep;
	...
	// private void recursiveVisit(AnnotatedElement ae, Set<Object> cache,
	// ChainedNode node, Filter filter)
	// {
	//
	...
	// }
	// else if (Class.class.isAssignableFrom(ae.getClass()) &&
	// ae.getClass().isAnnotation())
	// {
	...
	private void recursiveVisit(Object object, Set<Object> cache,
			Queue<LocalNode> nodes, Filter filter, int currentLevel) {
		currentLevel ++;
		if (!cache.contains(object)) {
			...
			if (object == null) {
				...
				visitAllCollection((Collection<?>) object, cache, nodes,
						currentLevel, filter);
			} else if (object.getClass().isArray()) {
				visitAllArray(object, cache, nodes, currentLevel, filter);
			} else if (Map.class.isAssignableFrom(object.getClass())) {
				visitAllMap((Map<?, ?>) object, cache, nodes, currentLevel,
						filter);
			} else {
	...
	private void visitObject(Object object, Set<Object> cache,
			 Queue<LocalNode> nodes, int currentLevel, Filter filter) {
		visitObject(object, cache, nodes, currentLevel, filter, null);
	}

	// private <T> void visitConstructor(Constructor<T> ae, Set<Object> cache,
	// ChainedNode node, int
	...
	private void visitObject(Object object, Set<Object> cache,
			Queue<LocalNode> nodes, int currentLevel, Filter filter, Metadata metadata) {
		...
		if (!isJdkMember(currentClass)) {
			...
			Class<?>[] family = getAllInterfacesAndClasses(currentClass);
			for (Class<?> elementClass : family) { // We iterate over all the
													// family tree of the
													// current class
													//
				if (elementClass != null && !isJdkMember(elementClass)) {

					for (Constructor<?> c : elementClass
					...
					for (Field f : elementClass.getDeclaredFields()) {
						if (!isJdkMember(f) && !f.isSynthetic()) {
							...
							if (filter != null && filter.retains(f)) {
								...
								recursiveVisit(deeperObject, cache, nodes,
										filter, currentLevel);
							}
						}
					}
	...
	private void visitAllCollection(Collection<?> collection,
			Set<Object> cache, Queue<LocalNode> nodes, int currentLevel, Filter filter) {

		boolean indexable = collection instanceof List
	...
	private void visitAllArray(Object arrayObject, Set<Object> cache,
			 Queue<LocalNode> nodes, int currentLevel, Filter filter) {
	...
	private void visitAllMap(Map<?, ?> values, Set<Object> cache,
			 Queue<LocalNode> nodes, int currentLevel, Filter filter) {
	...
	private boolean isJdkMember(Class<?> input) {
		return input.getPackage().getName().startsWith("java.")
				|| input.getPackage().getName().startsWith("javax.");
	}
	...
	/**
	 * Returns all the interfaces and classes implemented or extended by a
	 * class.
	 *
	 * @param clazz
